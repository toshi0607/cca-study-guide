...
- **学習パスの所要時間** — 各ステージが含む教材の分数を表示（Study Guide 360分・Hands-on 480分・模試120分）。所要時間が定まらない段階には表示しない
- **模試の既回答数** — 模試の開始画面に「この模試の問題のうち何問をすでにQuizで回答済みか」を事実として表示。初見で受けるタイミングを自分で選べる
- **確信度と部分正答** — 回答後に「確信あり / 迷った / 勘」を1タップで記録（任意・スキップ可）。複数選択で正解の一部だけを選び誤答を含まなかった場合は「一部正解」として区別して記録
- **受験予定日（任意）** — この端末にのみ保存し、エクスポートには含めない。残り日数と学習ガイドの未消化分数を表示する。準備状況の判断やペースの推奨は行わない
- **学習状況の要約コピー** — 弱点カード・低正答率の問題・復習期限の件数を短いテキストにまとめてクリップボードへコピー。外部には一切送信しない
- **ディープリンク** — Study Guide のセクション、ハンズオンのガイドとステップ、Practice カードに `#/...` の URL。ブックマーク・共有・別端末で同じ場所を開ける。URL に載るのはコンテンツ ID とビュー名だけで、進捗や回答は載せない
- **local-only storage** — 進捗はこのブラウザのlocalStorageだけに保存
- **JSON export/import** — 進捗の書き出し・読み込みで端末・ブラウザ間を移行。読み込み時に「置き換える」に加えて**統合する**を選べるので、2台の記録をどちらも失わずに合流できる。同じファイルを2回読み込んでも件数は増えない
