...
| 14 | q-d3-command-vs-skill | d3 | 3.2 | claude-code-configuration | single | foundation | commandはSkillへ統合済み。作り分けはSkillのfrontmatter（disable-model-invocation / user-invocable）。reviewer round2で最新仕様へ修正 | skills,code-features,code-best-practices(✓23) |
...
| 18 | q-d4-schema-design | d4 | 4.3 | structured-output | multiple | analysis | schema設計の規律：後段が使う項目に絞り過度なネストを避ける／additionalProperties:falseで想定外フィールド混入を防ぐ。既存structured-guaranteeは構文保証≠意味妥当（round1で重複回避rename、round2でchoice bをenum→additionalProperties:falseへ） | structured(✓23) |
...
- **Should-fix 重複3件**（配分quota上、既カバーobjectiveに追加が集中したことが一因）を、domainId/objectiveIdを変えずに別facetへ差し替えて解消:
  ...
  - #18 `q-d4-schema-semantic` → `q-d4-schema-design`: 構文保証≠意味妥当の言い換え → schema**設計規律**（最小化＋additionalProperties:false）へrename。
...
### レビュー round 2（Request changes）対応

人間レビュアーが公式ドキュメントとの整合で2件指摘。一次情報を再確認して対応。

- **Finding 2（Medium, 正当）q-d3-command-vs-skill が現行のcommands/Skillsモデルと不一致**: 公式skills docで「Custom commands have been merged into skills」を確認（`.claude/commands/deploy.md` と `.claude/skills/deploy/SKILL.md` は同じ /deploy を作る）。旧来の「command=明示/Skill=自動」二分法を、**両方Skill＋frontmatter（`disable-model-invocation: true` / `user-invocable: false`）で作り分け**る最新仕様へ設問・全rationale・explanationを書き換え。
- **Finding 1（Medium, 指摘は事実誤認だが設問は改善）q-d4-schema-design のenum保証**: 公式structured-outputs docは「Structured outputs **don't guarantee the capitalization** of string enum and const values ... Compare enum values case-insensitively」と明記しており、元のchoice b（enum大小非保証で比較は区別しない）は**docに合致し誤りではない**。ただし一意性・テーマ整合の観点から、choice bを同じくdocが強制を保証する **`additionalProperties: false`（想定外フィールドの混入防止）** へ差し替え、争点となるedge caseを排除。PR返信で一次情報を引用し記録。
