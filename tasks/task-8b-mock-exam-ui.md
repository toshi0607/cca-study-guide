...
- [x] 23. commit（3211d58）/ push / PR #38作成（マージしない）／CI全pass（lighthouse=perf権威 pass、playwright pass）
...
### 人間レビュー（Request changes）→ blocking 2件を修正（HEAD 3211d58 → 更新）

**B1: 保存失敗時にも結果画面へ遷移（exactly-once/storage-first違反）**。修正前は`runFinalize`が
`writeData`の戻り値を無視し、保存失敗でも「提出済み」結果を表示（activeMockExam残存・attempt未保存で
reload時に模試が復活）。修正: `runFinalize`が`{outcome, saved}`を返し、`saved`成功時のみresultへ遷移。
保存失敗時は新設の`save-error`画面（再試行 / submit時は受験に戻る）を表示し、`finalizingRef`解除で
再試行可能、active sessionを維持。E2E追加（setItem throwでsubmit→save-error・active残存・attempt 0件）。

**B2: stale attemptの復習が現在の問題内容と混在**。修正前は revision不一致でも stem/選択肢/正解/
explanation/rationale/source/内訳を現在の`questions`から表示し、古い正誤と矛盾。修正:
(1) result: stale時は content依存の domain/difficulty/skill 内訳を非表示（rawAccuracy等はstored由来で妥当・継続表示）。
(2) review: per-row で revision不一致/欠落の問題は保存された選択肢ID・正誤・回答のみ表示し、
現在のstem/正解/rationale/sourceを出さない。E2E追加（historyから stale attempt を開き内訳非表示・
復習で保存回答のみを確認）。

修正後 tsc clean / unit 376/376 / build exit0 / **full E2E 96/96**（+2件）再グリーン。
