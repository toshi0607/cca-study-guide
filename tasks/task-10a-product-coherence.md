...
### 再レビュー対応（PR #40 レビュー2回目, Request changes → 修正）
- **Medium-1**: Today の status-strip がカード集計なのに「この端末の進捗」で全体進捗に見える →
  `copy.status` を「カード練習の進捗 / Card-practice progress」（評価済み/未評価/収録カード）へ変更。
  `status.objectives`（30項目）を廃止し coverage=カード枚数に。E2E に「全体進捗と誤認させない」検証を追加。
- **Medium-2**: Guide の analysis stage CTA が attempt 0 でも「模試結果の分析を開く」表示だが landing フォールバック →
  `hasMockExamAttempts` を App→GuideEntry→GuideView へ渡し、attempt 0 のとき CTA を
  「模試を受けて分析を利用する / Take a mock exam to use learning analysis」に。挙動（landing 遷移）は維持。
  E2E に attempt 0/≥1 の CTA ラベルと遷移先の検証を追加。
- 非blocking: README ネストは既に 2 スペース（GitHub で正しくネスト表示）。Actions の Node20 警告は CI infra で scope 外。
