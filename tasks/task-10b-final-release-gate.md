...
### CI（PR #41 / 最新HEAD `02838fc`）
| check | 結果 |
|-------|------|
| playwright（full E2E + a11y, ubuntu） | **pass** (3m17s) |
| lighthouse（perf.yml, median of 3, ubuntu） | **pass** (1m28s) — レビュー#1のLCP境界フレークを spine で解消。eager −69% の余裕で clean 環境で確実に budget 内 |
| Vercel deploy | pass。Production=実ブラウザ確認済 / PR Preview=Vercel SSO(302)で直接不可・同一buildをローカル browser+131 E2E で検証 |

unit 423 / content 127(spine drift-guard含む) / full E2E 131 / astro check 0err / no-analytics / bundle guard(eager 9 chunks, none forbidden) すべて green。
