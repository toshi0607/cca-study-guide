...
# Round 2 (2026-07-16): Web Font Regression Fix

## Baseline

- The visual redesign (887e733) added a render-blocking `fonts.googleapis.com` stylesheet (57 KB; Zen Kaku Gothic New's unicode-range table) plus 16 font files / 161 KB, after the Round 1 measurements.
- Lighthouse 13.4.0, Chrome headless, mobile, production build + preview, GA `G-TEST123456`, Node 26.5.0, 3-run on `/`: Performance 62–70 (median 65); FCP ~4.6 s; LCP ~5.5 s; TBT 63–258 ms; CLS ~0; transfer 421 KB / 26 requests.
- render-blocking-insight attributed ~1,321 ms to the Google Fonts CSS. The LCP element remained `h2#today-title`, which uses the `--display` stack (Zen Kaku Gothic New 900 for Japanese).

## Change

- Self-hosted subset fonts generated by `scripts/subset-fonts.mjs` (`pnpm fonts:subset`): downloads the OFL TTFs from google/fonts, subsets with `subset-font` (harfbuzz), and writes `public/fonts/*.woff2` + `manifest.json` + OFL license copies. Outputs are committed, mirroring the `assets:generate` pattern, so builds stay offline.
- Subset corpus: printable ASCII, CJK punctuation, hiragana, katakana, full-width forms, every string literal in `src/i18n/ui.ts`, and the display-font text extracted from the built HTML. 609 characters; Barlow Condensed 700 = 16,920 B, Zen Kaku Gothic New 900 = 54,972 B (71.9 KB total vs 218 KB before).
- Unused weights Barlow 600 and Zen Kaku 700 are not shipped: `--display` is used only by `.wordmark b` (700, Latin "CCA") and `.today-hero h2, .page-header h2` (900).
- `LocalizedLayout.astro`: removed the Google Fonts stylesheet and both preconnects; inlined the two `@font-face` rules (`font-display: swap`); preloads `barlow-condensed-700.woff2` everywhere and `zen-kaku-gothic-new-900.woff2` on Japanese pages only.
- Guard: `src/lib/fonts.test.ts` recomputes the display-font characters from the i18n modules and fails when the committed manifest no longer covers them (message points to `pnpm build && pnpm fonts:subset`). The guard caught a real gap during development: hydration-only headings (guide/practice/progress) were missing from the dist-HTML extraction; the ui.ts literal extraction fixed it.
- No size-adjust fallback was added: measured CLS is 0.0000 in all runs without it.

## Matched A/B (mobile, 3-run median, GA enabled)

| Metric | Before | After | Delta |
|---|---:|---:|---:|
| Performance | 65 | 97 | +32 |
| FCP | 4,581 ms | 983 ms | -78.5% |
| LCP | 5,504 ms | 2,602 ms | -52.7% |
| TBT | 153 ms | 41 ms | -73.2% |
| CLS | 0.0000 | 0.0000 | 0 |
| Transfer | 421 KB | 273 KB | -148 KB |

- `/en/` single run: Performance 98; FCP 980 ms; LCP 2,455 ms; CLS 0.
- LCP breakdown after: TTFB 3 ms, element render delay 193 ms observed; the simulated residual is the immediate gtag (unused-JS ~66 KiB, unchanged standing decision) plus the font transfer itself. Round 1's font-less LCP was 2,292 ms, so ~310 ms is the accepted cost of keeping the blueprint typography.
- The Round 1 `unminified-css` flag was the Google Fonts CSS; it disappeared with self-hosting.

## Regression Guard

- `scripts/check-lighthouse-budget.mjs`: median-of-N budgets — Performance ≥ 90, FCP ≤ 2,000 ms, LCP ≤ 3,000 ms, CLS ≤ 0.02, and zero external stylesheets. Verified to pass on the after-reports and fail on the before-reports (score 65, FCP 4,581, LCP 5,504, external Google Fonts CSS all flagged).
- `.github/workflows/perf.yml`: on PR and main push — pnpm install, unit tests, production build with a test GA ID, preview, 3 Lighthouse runs, budget check, report artifacts.
