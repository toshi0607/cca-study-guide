...
The sketch above is the MVP shape. `src/content/types.ts` is the authoritative model and also carries the later additions: quiz questions (`ChoiceQuestion`, with a `difficulty` of `foundation` / `application` / `analysis` and `skills` drawn from the taxonomy in `src/content/skills.ts`), per-choice review copy (`ChoiceRationales` in `src/content/rationales.ts`, deliberately outside the question bank so the quiz island does not ship review-only text), fictional practice cases (`Scenario`), the six official exam scenarios used as a classification axis (`OfficialScenarioId`), the design-judgment learning layer over those six (`OfficialScenarioLearning`, in `src/content/official-scenarios.ts`), and the study-guide and hands-on models (`StudyGuideSection`, `HandsOnGuide`). Domain, task statement, and skill stay distinct axes: a domain carries exam weight, a task statement names an area of the guide, and a skill names the capability a question measures. Validation covers every one of these types, including reference integrity between them. Beyond reference integrity, `validateStudyGuideLinkCoverage` (kept apart from per-entry validation, like `validateHandsOnThemes`) enforces link *completeness*: every card — and every standalone question — whose objectives fall inside a study-guide section's task statements must appear in that section's `relatedCardIds` / `relatedQuestionIds`, so the natural "read the section, then work its related material" flow cannot silently skip in-scope items when new content is added. Scenario-bound questions are never linked directly (a direct link would strip their scenario context) — the check rejects a `relatedQuestionIds` entry pointing at one; instead it enforces their reachability through `relatedScenarioIds`: every scenario carrying an in-scope question must appear there — rendered as scenario links in the guide section — and every listed scenario must carry at least one in-scope question, so a section can neither strand a scenario question, bypass a scenario's context, nor link an unrelated scenario.
...
**Returning from a section excursion.** A related card, question, or scenario opened from a Study Guide section leaves the section behind: the target view replaces it, and the learner who wanted "read the section, then work its related material" had no way back except re-finding the section by hand. The navigation that carries a target now also carries an **origin** — `GuideOrigin` (`{ sectionId, title }`), set only by the Study Guide's related-item links — and Practice and Quiz render it as a back link that reopens the section exactly as a deep link would (expanded, scrolled to, summary focused). It rides `navigate` for the same reason the target does: every navigation clears it unless that navigation names one, so an origin can never leak into an unrelated view. The localized title travels with the id so neither view has to import the guide content. The origin is independent of the single-card filter, so dismissing "show all cards" does not take the way back with it; a running practice session hides it, having replaced the list it belongs to.

**Notice lifetime.** The aria-live notice stays mounted (a live region that appears late is not reliably announced), but its text does not stay forever: a confirmation clears itself after eight seconds and any navigation clears it immediately, because a notice describes what just happened on the view it was raised on. Failure notices are `sticky` and ignore both rules — a message the learner has to act on (a refused save, an invalid import, a partial reset) must not disappear while they are reading it, so a navigation drops only the transient ones. Work that finishes after the learner has moved on stays silent either way: an async completion (the clipboard copy) captures a monotonic navigation count when it starts and announces only if that count has not changed — a view-name comparison would mistake leaving and coming back for never having left.
...
**WebMCP tools.** The pasteable summary still needs the learner to carry it; an agent running in the browser can instead ask the open tab. `src/lib/webmcp/webmcp-tools.ts` registers six tools on `document.modelContext` — the W3C WebMCP draft API through which an in-browser agent (a browser's built-in assistant, or an MCP host such as Claude reached through the WebMCP extension or local relay) discovers and calls page functions. Five are reads marked `readOnlyHint`: `get_study_summary` wraps `buildStudySummary`; `get_due_reviews`, `get_domain_stats`, `search_content`, and `get_content_item` return due cards, per-domain counts, keyword hits, and one item's text. One navigates: `open_view` builds the equivalent hash and runs the same `parseDeepLink` → `applyDeepLink` path a typed URL takes, so a crafted id reaches the views through the tool no more than through the address bar; it refuses an id on a view that has no addressable item rather than echoing back a target that was never used, and it will not leave the Practice view while a review session is running, because leaving ends the session and that is the learner's call. The rule that shapes all of them: a tool reads the state App renders and navigates through App's own functions, never localStorage directly, so it can neither desynchronise the UI nor bypass the storage validator. Outputs are raw counts and content ids in the summary's stable English vocabulary, held inside Chrome's 1.5K-character guidance as measured on the serialised result of every call, success or error: text fields are clipped, a list is shortened (and says `truncated`) until the result fits, the digest's id lists shrink step by step before anything is cut, echoed inputs are bounded (`query` to 120 characters; ids must match the deep-link id pattern and an invalid one is described, never repeated), and a final guard replaces anything that still would not fit with an error — all asserted against the shipped content at both default and maximum limits; no tool derives a score, pass/fail, or readiness; quiz questions come back without their correct choices or explanation so `quizStats` keeps its meaning; and the mock exam has no tool at all, not even navigation — an agent that could take it would defeat it, and merely mounting its view finalises an expired session, which is a write. `tests/webmcp.spec.ts` opens every reachable view through the tool against a seeded record and asserts the stored document is byte-identical afterwards, because a "no writes" rule checked only by grepping for `save()` cannot see a write that a mounted view performs. Tools that write (recording a rating, completing a section) are deliberately absent from this phase. `useWebMcp` registers after the island is ready and the browser idle (bounded by a two-second timeout so a busy tab still registers): the module and the bundled bridge (`@mcp-b/global`, which polyfills the API where the browser lacks it, mirrors the object onto the legacy `navigator.modelContext` alias, and exposes the tools to MCP hosts over this window's own `postMessage`, accepting messages from this origin only) are dynamic imports, kept out of the initial bundle by `check-initial-bundle.mjs` — which searches the eager graph for markers of both rather than trusting the bundler's chunk names. Content modules are fetched by the first tool call that needs each of them, not at registration. One `AbortController` per mount is the only way to unregister, because the API has none; aborting also tears the bridge down. The bridge is initialised before any tool is registered so the registrations are visible through it. Everything runs inside the page and issues no request; what an agent does with a result is that agent's business, exactly as with the clipboard summary. `tests/webmcp.spec.ts` pins registration on a native-shaped API, a read and a navigation round-trip, the bridge path a browser without WebMCP takes, the MCP wire an extension speaks over `postMessage`, and that no request leaves the origin; `tests/webmcp-native.spec.ts` runs Chromium with its WebMCP flag and lets the browser's own tool runner list and execute the tools. The local-relay route (`@mcp-b/webmcp-local-relay`) is deliberately unsupported: it needs a CDN script, a hidden iframe, and a loopback WebSocket, each of which the CSP forbids.
...
- WebMCP tools (`document.modelContext`) for agents running in the learner's browser, registered lazily by the App island over the same state and deep-link paths; `@mcp-b/global` is the one runtime dependency added for it (see Study companion affordances).
