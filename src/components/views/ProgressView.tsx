...
export function ProgressView({
  ...
  analyticsEnabled, dataUnreadable, examDate, onExamDateChange, onExamDateClear, onExport, onCopySummary, onImportFile, onReset,
  ...
  return (
    <section class="progress-view" aria-labelledby="progress-title">
      ...
      />
      <section class="exam-date-panel panel" aria-labelledby="exam-date-title">
        <h3 class="section-title" id="exam-date-title">{copy.progress.examDateTitle}</h3>
        <p>{copy.progress.examDateDescription}</p>
        <div class="data-actions">
          <label class="exam-date-label" for="exam-date-input">{copy.progress.examDateLabel}</label>
          <input id="exam-date-input" class="exam-date-input" type="date" value={examDate ?? ''} onChange={(event) => onExamDateChange((event.currentTarget as HTMLInputElement).value)}/>
          {examDate && <button type="button" class="btn btn--secondary exam-date-clear" onClick={onExamDateClear}>{copy.progress.examDateClear}</button>}
        </div>
      </section>
      <section class="data-panel panel" aria-labelledby="data-title"><div><h3 class="section-title" id="data-title">{copy.progress.localData}</h3><p>{copy.progress.localDataDescription}</p><p>{copy.progress.copySummaryDescription}</p>{analyticsEnabled && <p class="note note--info">{copy.progress.analyticsDisclosure}<a href={localePaths[locale].privacy}>{copy.progress.details}</a></p>}</div><div class="data-actions"><button class="btn progress-copy-summary" onClick={onCopySummary} disabled={dataUnreadable} aria-describedby={dataUnreadable ? 'data-unreadable-actions-note' : undefined}>{copy.progress.copySummary}</button><button class="btn" onClick={onExport} disabled={dataUnreadable} aria-describedby={dataUnreadable ? 'data-unreadable-actions-note' : undefined}>{copy.progress.exportJson}</button><button class="btn" onClick={() => importInputRef.current?.click()}>{copy.progress.importJson}</button><input ref={importInputRef} type="file" accept=".json,application/json" hidden onChange={onImportFile}/><button class="btn btn--danger" onClick={onReset} disabled={dataUnreadable} aria-describedby={dataUnreadable ? 'data-unreadable-actions-note' : undefined}>{copy.progress.reset}</button></div>{dataUnreadable && <p id="data-unreadable-actions-note" class="note note--danger">{copy.progress.dataUnreadableActions}</p>}</section>
