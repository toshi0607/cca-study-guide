...
export function MockExamSubmitDialog({ open, answered, unanswered, flagged, submitting, copy, onConfirm, onCancel }: {
  ...
  return (
    <dialog ref={dialogRef} class="mock-exam-submit-dialog" aria-labelledby="mock-exam-submit-title" onClose={onCancel} onCancel={onCancel}>
      <h2 id="mock-exam-submit-title">{copy.mockExam.submitDialogTitle}</h2>
      ...
      <p class="mock-exam-submit-warn">{copy.mockExam.submitDialogWarnNoChange}</p>
      {unanswered > 0 && <p class="mock-exam-submit-warn" role="note">{copy.mockExam.submitDialogWarnUnanswered}</p>}
      <div class="mock-exam-dialog-actions">
        <button type="button" class="mock-exam-secondary" onClick={onCancel} disabled={submitting}>{copy.mockExam.submitDialogCancel}</button>
        <button type="button" class="mock-exam-primary" onClick={onConfirm} disabled={submitting}>{copy.mockExam.submitDialogConfirm}</button>
      </div>
    </dialog>
  );
}
