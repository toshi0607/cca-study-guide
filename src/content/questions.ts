...
export const questions: ChoiceQuestion[] = [
  ...
  question(
    ...
    { difficulty: 'foundation', skills: ['claude-code-configuration'] },
    {
      stem: 'チームで再利用する手順を用意します。利用者が明示的に起動する定型操作と、必要に応じて自動的に読み込まれる知識の使い分けとして最も適切なのはどれですか？',
      choices: [
        'どちらもコマンドにし、Skillは使わないでおく',
        'どちらもSkillにし、利用者は起動を意識しない',
        '手順はすべてCLAUDE.mdへ書き、コマンドもSkillも使わない',
        '利用者が起動する定型操作はコマンドにし、状況に応じて参照される知識はSkillにする',
      ],
      explanation: '明示的に呼ぶ定型操作はコマンド、必要時に自動で読み込まれる再利用知識はSkillが適します。全部を一方に寄せると起動方法や読み込みタイミングが合わず、全部CLAUDE.mdは常時読み込みで肥大化します。',
    },
    {
      stem: 'You are preparing reusable procedures for a team. What is the most appropriate split between a routine the user invokes explicitly and knowledge loaded automatically when relevant?',
      choices: [
        'Make both commands and do not use Skills',
        'Make both Skills so the user never thinks about invoking them',
        'Put every procedure in CLAUDE.md and use neither commands nor Skills',
        'Make the user-invoked routine a command, and the situationally-referenced knowledge a Skill',
      ],
      explanation: 'An explicitly invoked routine fits a command, while reusable knowledge loaded on demand fits a Skill. Forcing everything into one mismatches how it is invoked or loaded, and putting it all in CLAUDE.md bloats every session since it always loads.',
    },
  ...
  question(
    ...
    { difficulty: 'analysis', skills: ['structured-output'] },
    {
      ...
      choices: [
        ...
        '列挙(enum)値は大文字小文字の揺れが保証されないため、比較は大文字小文字を区別せずに行う',
        '想定し得る全項目を最初から網羅し、深くネストするほど厳密で良いスキーマになる',
        '業務ルールをすべてenumやパターンで表現すれば、アプリ側の値検証は不要になる',
      ],
      explanation: 'スキーマは後段が必要とする項目に絞り、過度な複雑さを避けるのが保守的です。列挙値の大文字小文字は保証されないため比較は区別せず行います。全項目の網羅や過度なネスト、業務ルールの全表現は、脆さやアプリ側検証の欠落を招きます。',
    },
    {
      ...
      choices: [
        ...
        'Because the capitalization of enum values is not guaranteed, compare enum values case-insensitively',
      ...
      explanation: 'Keep the schema to the fields the downstream needs and avoid excess complexity, and because enum capitalization is not guaranteed, compare enum values case-insensitively. Covering every field, over-nesting, or encoding all business rules invites brittleness or a gap in application-side validation.',
    },
    ['structured'],
    { verifiedAt: EXPANSION_VERIFIED_AT },
  ),
