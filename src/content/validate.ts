...
export function validateContent() {
  ...
  const scenarioResult = z.array(scenarioSchema).safeParse(scenarios);
  ...
  if (!scenarioResult.success) errors.push(scenarioResult.error.message);
  ...
  unique(scenarios.map((x) => x.id), 'scenarios', errors);
  ...
  for (const item of [...domains.flatMap((d) => d.objectives), ...cards, ...questions, ...scenarios]) {
  ...
  const scenarioDomainIds = new Map(scenarios.map((scenario) => [scenario.id, new Set(scenario.domainIds)]));
  for (const scenario of scenarios) {
    for (const domainId of scenario.domainIds) if (!domainIds.has(domainId)) errors.push(`${scenario.id}: orphan domain ${domainId}`);
  }
  for (const question of questions) {
    if (!question.scenarioId) continue;
    if (!scenarioIds.has(question.scenarioId)) errors.push(`${question.id}: orphan scenario ${question.scenarioId}`);
    else if (!scenarioDomainIds.get(question.scenarioId)!.has(question.domainId)) errors.push(`${question.id}: domain ${question.domainId} is not declared on scenario ${question.scenarioId}`);
  }
  for (const scenario of scenarios) {
    const linked = questions.filter((question) => question.scenarioId === scenario.id);
    if (linked.length < 3 || linked.length > 5) errors.push(`${scenario.id}: needs 3-5 questions, has ${linked.length}`);
    if (!linked.some((question) => question.format === 'single') || !linked.some((question) => question.format === 'multiple')) {
      errors.push(`${scenario.id}: needs both single and multiple questions`);
    }
  }
  for (const question of questions) {
  ...
  return { sourceCount: sources.length, domainCount: domains.length, objectiveCount: objectiveIds.size, cardCount: cards.length, questionCount: questions.length, scenarioCount: scenarios.length };
}
