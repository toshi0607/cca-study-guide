...
export const cards: Card[] = [
  ...
  card(
    'd1-hook-exit-codes', 'd1', ['1.5'], 'recall',
    {
      ...
      answer: '0は「異議なし」で処理は通常どおり進む（PreToolUseでは通常の権限フローが引き続き適用される）。2はブロックで、JSONに理由がなければstderrへ書いた内容がそのままブロック理由になる。',
      ...
      pitfall: '終了コード1ではブロックできません。ポリシーを強制するなら2を使います。なお終了コード2はJSONを出していてもブロックし、JSONのpermissionDecision: "allow"では覆せません。JSON自体が無視されるわけではなく、理由を書いていればブロック理由の文言として使われます。',
    },
    {
      prompt: 'In a command hook, what do exit codes 0 and 2 mean?',
      answer: 'Exit 0 means “no objection” and processing continues normally (for PreToolUse, the regular permission flow still applies). Exit 2 blocks the action; unless your JSON supplies a reason, whatever you wrote to stderr becomes the blocking message.',
      ...
      pitfall: 'Exit code 1 does not block — use exit 2 to enforce policy. Note also that exit 2 blocks even when you print JSON, and a JSON permissionDecision of “allow” cannot override it. The JSON is not ignored, though: a reason inside it is used as the blocking message.',
    },
