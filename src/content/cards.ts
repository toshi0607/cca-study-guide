...
export const cards: Card[] = [
  ...
  card(
    'd2-config-scopes', 'd2', ['2.4'], 'recall',
    {
      ...
      answer: 'local（自分×現在のプロジェクト）、project（.mcp.jsonでチーム共有）、user（自分×全プロジェクト）。優先度は local > project > user で、同名なら最も優先度の高いスコープ（local が最優先）の定義が丸ごと使われる。',
      explanation: '共有する.mcp.jsonでは ${VAR} や ${VAR:-default} の環境変数展開が使え、APIキーなどの秘密は設定ファイルへ直書きせず環境変数から注入します。',
      pitfall: 'スコープをまたいだフィールド単位のマージは行われません。優先度の高い側の定義が全体を置き換えます。',
    },
    {
      ...
      answer: 'local (you, current project), project (team-shared via .mcp.json), and user (you, all projects). Precedence is local > project > user, so a duplicate name resolves to the entire definition from the highest-precedence scope that has it (local first).',
      ...
      pitfall: 'Fields are not merged across scopes. The higher-precedence definition replaces the whole entry.',
    },
    ['code-mcp'],
    EXPANSION_VERIFIED_AT,
  ),
